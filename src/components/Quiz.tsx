...
function Quiz() {
  ...
  const getRandomWords = () => {
    if (wordsRemaining?.current?.length > 0) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        randomWordIndex = Math.floor(
          Math.random() * wordsRemaining.current.length
        );
        quiz.length > 0 &&
          quiz.forEach((word) => {
  ...
  if (quiz.length > 0) {
    const allWordsAnsweredCorrect = quiz.every(
      (obj) => obj.answered === "correct"
    );
  ...
  const updateQuiz = (index, updatedAnswered) => {
    setQuiz((prevQuiz) =>
      prevQuiz.map((obj) =>
        obj.index === index ? { ...obj, answered: updatedAnswered } : obj
      )
    );
  };
